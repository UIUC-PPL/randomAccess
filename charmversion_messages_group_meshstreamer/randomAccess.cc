...
CProxy_Updater updater_array_nondelegated;
/* Readonly variables */
...
public:
    Main(CkArgMsg* args) 
    {
        ...
        MeshStreamingStrategy *strategy = new MeshStreamingStrategy(PERIOD_IN_MS, NMSGS);
        stratStreaming = ComlibRegister(strategy);
        updater_array_nondelegated = updater_array;
        ComlibAssociateProxy(stratStreaming, updater_array); 
        updater_array_nondelegated.initialize();
    ...
    void allUpdatesDone(DUMMYMSG *msg)
    {
        ...
        {
            updater_array_nondelegated.checkErrors();
            //verify done
